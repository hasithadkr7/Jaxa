...
schedule_interval = '02,32 * * * *'
dag_pool = 'jaxa_sat_pool'
...
cmd_str_d03 = 'python3 %s/curw/rainfall/wrf/extraction/sat_extractor_half_hr.py ' \
...
cmd_str_d01 = 'python3 %s/curw/rainfall/wrf/extraction/sat_extractor_half_hr.py ' \
              '-end {{ macros.datetime.strftime(execution_date + macros.timedelta(hours=1), \'%%Y-%%m-%%d_%%H:%%M\') }} ' \
              '-start {{ execution_date.strftime(\'%%Y-%%m-%%d_%%H:%%M\') }} ' \
